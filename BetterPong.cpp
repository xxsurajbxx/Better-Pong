...
struct Entity {    float x, y;
    ...
    char character;
    bool loss=false;
    Entity(float tx, float ty, int tlength, char charact) : x(tx), y(ty), length(tlength), character(charact)  {}
    static void addEntity(char* array, Entity e) {
        for(int i=(int)e.y; i<(int)e.y+e.length; i++) {
            array[(int)((i-(e.length/2))*WIDTH+e.x)]=e.character;
        }
    }
};
...
int main() {
    ...
    while(!loss) {
        ...
        ball.x+=deltaX;
        ball.y+=deltaY;
        if(ball.x<=1.5||ball.x>=118.5) {
            if(ball.y<paddle_right.y+(paddle_right.length/2)&&ball.y>paddle_right.y-(paddle_right.length/2)) {                if((int)(ball.y-paddle_right.y)==0) {deltaX==-1;}
                else {                    deltaX*=-1/std::pow(std::abs(ball.y-paddle_right.y),2);                    deltaY*=std::abs(ball.y-paddle_right.y);                }
            }            else if(ball.y<paddle_left.y+(paddle_left.length/2)&&ball.y>paddle_left.y-(paddle_left.length/2)) {                if((int)(ball.y-paddle_left.y)==0) {deltaX==1;}
                else {
                    deltaX*=-1/std::pow(std::abs(ball.y-paddle_left.y),2);
                    deltaY*=std::abs(ball.y-paddle_left.y);
                }            }
            else {
